...
func GitApprovalWorkflow(ctx workflow.Context, owner, reponame, token string, aks models.AKS) (string, error) {
	...
	err := workflow.ExecuteActivity(ctx, a.GitCreateTicketActivity, owner, reponame, token, aks.AKSClusterName, aks.SubscriptionID, aks.Location, aks.AKSAppName).Get(ctx, &ticketNumber)
	if err != nil {
		return "", err
	}
